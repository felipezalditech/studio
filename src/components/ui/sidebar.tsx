...
>(
  (
    ...
    React.useEffect(() => {
        if (isMobile) {
            setOpen(false); // Collapse on mobile by default
        } else {
            // Restore from cookie or use default for desktop
            const cookieValue = typeof window !== 'undefined' ? document.cookie
                .split('; ')
                .find(row => row.startsWith(`${SIDEBAR_COOKIE_NAME}=`))
                ?.split('=')[1] : undefined;
            if (cookieValue) {
  ...
  (
    {
      side = "left",
      variant = "sidebar",
      collapsible = "offcanvas", // Default to offcanvas for mobile, can be "icon" for desktop
      className,
      children,
      ...props
    },
  ...
  return (
    <main // Changed from div to main
      ...
      className={cn(
        ...
        // Adjust margin based on sidebar state for desktop view
        !isMobile && state === "expanded" ? "md:ml-[--sidebar-width]" : "md:ml-[--sidebar-width-icon]",
        // Full width on mobile or when sidebar is offcanvas collapsed
  ...
  return (
    <Input
      ...
      className={cn(
        ...
        "group-data-[state=collapsed]:hidden", // Hide input when sidebar is collapsed
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex flex-col", className)} // Removed default padding and gap
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("flex flex-col mt-auto", className)} // Added mt-auto, removed default padding and gap
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto overflow-x-hidden", // Changed gap, added overflow-x-hidden
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    data-sidebar="menu"
    className={cn("flex w-full min-w-0 flex-col gap-0.5", className)} // Changed gap
    {...props}
  />
))
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-colors duration-150 ease-in-out hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-primary data-[active=true]:font-medium data-[active=true]:text-sidebar-primary-foreground group-data-[state=collapsed]:justify-center group-data-[state=collapsed]:p-2 group-data-[state=collapsed]:h-10 group-data-[state=collapsed]:w-10 [&>span:last-child]:group-data-[state=collapsed]:hidden [&>svg]:size-5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
      ...
      size: {
        default: "h-10 text-sm", // Adjusted height and text size
        sm: "h-9 text-xs",
        lg: "h-12 text-base group-data-[state=collapsed]:!p-0", // Adjusted for larger items
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      "ml-auto flex h-5 min-w-5 items-center justify-center rounded-md px-1.5 text-xs font-medium tabular-nums text-sidebar-primary-foreground bg-sidebar-primary select-none pointer-events-none",
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "ml-4 flex min-w-0 translate-x-px flex-col gap-0.5 border-l border-sidebar-border pl-3 py-1", // Adjusted padding and margin
      "group-data-[state=collapsed]:hidden",
      className
    )}
    {...props}
...
const SidebarMenuSubButton = React.forwardRef<
  HTMLAnchorElement, // Usually an anchor tag for navigation
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  const Comp = asChild ? Slot : "a" // 'a' for anchor
